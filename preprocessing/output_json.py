...
def read_dataset(file_name, data_type):
    ...
    with open(file_name, "r") as f:
        for line in f:
            ...
            if (dataset_name == 'YaHooC14B') and (data_type == 'Train'):
            ...
            elif (dataset_name == 'YaHooC14B') and (data_type != 'Train'):
                raw_feature = {fe.split(':')[0]:fe for fe in items[2:]}
                for idx in range(700):
                    cur_id = idx + 1
                    if str(cur_id) in raw_feature.keys():
    ...
    print(file_name)
    print("0标签有" + str(is_zero))
    print("1标签有" + str(is_one))
    print("2标签有" + str(is_two))
    print("3标签有" + str(is_three))
    print("4标签有" + str(is_four))
    print()
    return each_row
...
def handle_data(file_path):
    ...
    # Min-Max Scale for dataset
    # print('Doing Min-Max Feature Scale...')
    # train_dat = pd.read_json(out_file_path + 'Train' + ".json")
    # vali_dat = pd.read_json(out_file_path + 'Vali' + ".json")
    # test_dat = pd.read_json(out_file_path + 'Test' + ".json")
    # train_dat_scale, vali_dat_scale, test_dat_scale = scale_dataset(train_dat,vali_dat,test_dat)
    # train_dat_scale.to_json(out_file_path + 'Train' + ".json")
    # vali_dat_scale.to_json(out_file_path + 'Vali' + ".json")
    # test_dat_scale.to_json(out_file_path + 'Test' + ".json")
