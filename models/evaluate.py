...
class Evaluator(object):
    ...
    def _get_predicts(self, model):
        ...
        return eval_predicts.view(-1).tolist()
...
class Test_Evaluator(Evaluator):
    ...
    def evaluate_linear(self, param):
        # eval_predicts = super(Test_Evaluator,self)._get_predicts(param)
        
        eval_feature = np.array(self.eval_log['feature'].values.tolist())
        eval_predicts = norm.cdf(np.matmul(eval_feature, param))
        df_eval, df_stat = super(Test_Evaluator,self)._get_stat(eval_predicts)
    ...
    def evaluate_agg(self, model, param):
        # eval_predicts = super(Test_Evaluator,self)._get_predicts(param)
        
        eval_feature = np.array(self.eval_log['feature'].values.tolist())
        eval_predicts_heckman = norm.cdf(np.matmul(eval_feature, param))
        eval_predicts_ips = super(Test_Evaluator,self)._get_predicts(model)
        df_eval, df_stat = self._get_stat_agg(eval_predicts_ips, eval_predicts_heckman)
    ...
    def _get_stat_agg(self, pred_ips, pred_heckman):
        # build evaluation dataframe
        df_eval = pd.DataFrame([], columns=['qid', 'did', 'label', 'predict'])
        df_eval['qid'] = self.eval_log['qid']
        df_eval['did'] = self.eval_log['did']
        df_eval['label'] = self.eval_log['label']
        df_eval['predict_ips'] = pred_ips
        df_eval['predict_heckman'] = pred_heckman
        ct = df_eval.groupby('qid').apply(lambda part: len(part)).reset_index()
        df_eval = pd.merge(df_eval, ct, on='qid')
        df_eval.rename(columns={0:'rank_len'},inplace = True)
        # give corresponding rank position for each query-doc pair
        df_eval['rank_ips'] = df_eval.groupby('qid')['predict_ips'].rank(method='first', ascending=False)
        df_eval['rank_heckman'] = df_eval.groupby('qid')['predict_heckman'].rank(method='first', ascending=False)
        borda_score = df_eval.apply(lambda row: 2 * row['rank_len'] - row['rank_ips'] - row['rank_heckman'],axis=1)
        df_eval['borda_score'] =  borda_score
        df_eval['rank'] = df_eval.groupby('qid')['borda_score'].rank(method='first', ascending=False)
        # sort for each group
        temp = df_eval.groupby('qid').apply(lambda x: x.sort_values('rank', ascending=True))
        temp.reset_index(drop=True, inplace=True)
        # output click_list/ips_list/rank_list for each session
