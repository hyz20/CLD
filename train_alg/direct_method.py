...
from models.toy_model import rank_model,rank_model2


class Direct_Method(Learning_Alg):
    def __init__(self, args):
        super(Direct_Method, self).__init__(args)
        self.rel_estimator = args.rel_estimator
        self.pairwise = True

    def train(self):
        self.test_tool, self.in_size = super(Direct_Method, self)._load_test_and_get_tool()
        ...
        self.model, self.optim, self.scheduler, self.early_stopping = super(Direct_Method, self)._init_train_env()
        self._train_iteration(input_train_loader_topK, input_train_loader_outK)
        super(Direct_Method, self)._test_and_save()
    ...
    def _load_and_wrap_train(self, train_log_estimate):
        if self.pairwise:
            ...
            #                                     rel_diff=torch.Tensor((train_log_pair_topk['rel_diff'] * train_log_pair_topk['dm_weight']).to_list()))
            # else:
            ...
            #                                         rel_diff=torch.Tensor((train_log_pair_outk['rel_diff'] * train_log_pair_outk['dm_weight']).to_list()))
            # else:
    ...
    def _get_pair(self, df, mode='train'):
        ...
        df_sort_topk = df[df['rankPosition']<self.topK].groupby('sid').apply(lambda x: x.sort_values(by = ['dm_score'], ascending=[False]))
        df_sort_topk.reset_index(drop=True, inplace=True)    
        df_sort_outk = df[df['rankPosition']>=self.topK].groupby('sid').apply(lambda x: x.sort_values(by = ['dm_score'], ascending=[False]))
        ...
        for _, row_out in tqdm(df_sort_topk.iterrows(), total=df_sort_topk.shape[0]):
            if row_out['isClick'] > 0:
                for _, row_in in df_sort_topk[df_sort_topk['sid']==row_out['sid']].iterrows():
                    if row_out['dm_label']*row_out['dm_score'] > row_in['dm_label']*row_in['dm_score']:
        ...
        df_sort_outk = df_sort_outk.groupby('sid').apply(lambda x: x.sort_values(by = ['dm_label'], ascending=[False]))
        df_sort_outk.reset_index(drop=True, inplace=True)
        df_sort_outk = df_sort_outk[df_sort_outk['estimate_var']<=0.0010]

        up_quantiles = df_sort_outk.groupby('sid')['dm_label'].quantile(0.95)
        low_quantiles = df_sort_outk.groupby('sid')['dm_label'].quantile(0.05)
        ...
        for sid in tqdm(all_sid, total=len(all_sid)):
            ...
            length = cur_part.shape[0]
            start_idx = cur_part.iloc[0].name
            if (cur_part.head(1)['dm_label'].values - cur_part.tail(1)['dm_label'].values)[0] >= threshold_quantiles[sid]:
                for idx, row_out in cur_part.iterrows():
                    cur_idx = idx
                    rows_left = list(filter(lambda x: x is not None, [r if (row_out['dm_score']*row_out['dm_label'] - r['dm_score']*r['dm_label']) >= threshold_quantiles[sid] else None 
    ...
    def _assign_val(row_out, row_in, isTopK=True, mode='train'):
        ...
        if mode=='train':
            row_new = [ row_out['sid'],
                        tag,
                        # (row_out['dm_score']*row_out['dm_label'] - row_in['dm_score']*row_in['dm_label']),
                        (row_out['normalized_var'] * row_in['normalized_var']) * (row_out['dm_score']*row_out['dm_label'] - row_in['dm_score']*row_in['dm_label']) * row_out['dm_weight'], 
                        row_out['did'],
                        row_in['did']]
        elif mode=='vali':
            row_new = [ row_out['sid'], 
    ...
    def _pre_train_and_estimate(self):
        ...
        if self.rel_estimator=='random':
            ...
            train_log['dm_label'] = label_train
            train_log['dm_score'] = dr_score_train
            train_log['dm_weight'] = lamda_train

            vali_log['dm_label'] = label_vali
            vali_log['dm_score'] = dr_score_vali 
            vali_log['dm_weight'] = lamda_vali       
            
            #train_log.to_json('../datasets/result1/train_log_rand_estimate.json')
            #vali_log.to_json('../datasets/result1/vali_log_rand_estimate.json')
            ...
            ips_scheduler = ReduceLROnPlateau(ips_optim, 
                                        ...
                                        mode=self.schedul_mode,
                                        verbose=True)
            ips_early_stopping = EarlyStopping(fp=self.fout, patience = 3, verbose=True) 
            ...
            for epoch in range(self.epoch):
                ...
                for _id, batch in enumerate(ips_input_train_loader):
                    ...
                    MSE_lossfunc = Unbias_MSELoss_reg(weight=batch[2])
            ...
            train_log.to_json('../datasets/result1/train_log_ips_estimate.json')
            vali_log.to_json('../datasets/result1/vali_log_ips_estimate.json')
