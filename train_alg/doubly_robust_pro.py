...
from utils.data_wrapper import Wrap_Dataset_clickLog, Wrap_Dataset_fullInfo, Wrap_Dataset_Pairwise, Wrap_Dataset_Pairwise2, Wrap_Dataset_clickLog2
...
from models.loss_func import BPRLoss, HingeLoss, Unbias_MSELoss,BPRLoss_log_norm, Unbias_MSELoss_reg,Tobit_MLELoss
from models.toy_model import rank_model, rank_model2, rank_model2_tobit


class Doubly_Robust_pro(Learning_Alg):
    def __init__(self, args):
        super(Doubly_Robust_pro, self).__init__(args)
        self.rel_estimator = args.rel_estimator
        self.pairwise = True

    def train(self):
        self.test_tool, self.in_size = super(Doubly_Robust_pro, self)._load_test_and_get_tool()
        ...
        self.model, self.optim, self.scheduler, self.early_stopping = super(Doubly_Robust_pro, self)._init_train_env()
        self._train_iteration(input_train_loader_topK, input_train_loader_outK)
        super(Doubly_Robust_pro, self)._test_and_save()
    ...
    def _load_vali_and_get_tool(self, vali_log_estimate):
        if self.pairwise:
            ...
            vali_tool = Vali_Evaluator(vali_log_estimate[vali_log_estimate['rankPosition']<10],
                                       self.eval_positions, 
    ...
    def _load_and_wrap_train(self, train_log_estimate):
        if self.pairwise:
            ...
            #                                         rel_diff=torch.Tensor(train_log_pair_outk['rel_diff'].to_list()))
            # else:
    ...
    def _get_pair(self, df, mode='train'):
        ...
        for _, row_out in tqdm(df_sort_topk.iterrows(), total=df_sort_topk.shape[0]):
            if row_out['isClick'] > 0:
                for _, row_in in df_sort_topk[df_sort_topk['sid']==row_out['sid']].iterrows():
                    if row_out['dr_score']*row_out['dr_label'] > row_in['dr_score']*row_out['dr_label'] :
        ...
        df_sort_outk.reset_index(drop=True, inplace=True)
        df_sort_outk = df_sort_outk[df_sort_outk['estimate_var']<=0.010]
    ...
    def _assign_val(row_out, row_in, isTopK=True, mode='train'):
        ...
        if mode=='train':
            row_new = [ row_out['sid'],
                        tag,
                        # (row_out['dr_score']*row_out['dr_label'] - row_in['dr_score']*row_in['dr_label']),
                        (row_out['normalized_var'] * row_in['normalized_var']) * (row_out['dr_score']*row_out['dr_label'] - row_in['dr_score']*row_in['dr_label']) * row_out['dr_weight'], 
                        row_out['did'],
                        row_in['did']]
        elif mode=='vali':
            row_new = [ row_out['sid'], 
    ...
    def _pre_train_and_estimate(self):
        ...
        if self.rel_estimator=='random':
            ...
            hasClick_dict =vali_log.groupby(['qid','did'])['isClick'].apply(sum).reset_index()
            hasClick_dict.rename(columns={'isClick':'click_sum'},inplace = True)
            mg = vali_log.merge(hasClick_dict, on=['qid','did'])
            mg['use'] = mg.apply(lambda row: self._check(row),axis=1)

            #vali_log_topK = mg[(mg['rankPosition'] < self.topK) & (mg['use']==1)]
            vali_log_topK = mg[(mg['rankPosition'] < self.topK)]
            ...
            hasClick_dict =train_log.groupby(['qid','did'])['isClick'].apply(sum).reset_index()
            hasClick_dict.rename(columns={'isClick':'click_sum'},inplace = True)
            mg = train_log.merge(hasClick_dict, on=['qid','did'])
            mg['use'] = mg.apply(lambda row: self._check(row),axis=1)

            #train_log_topK = train_log[train_log['rankPosition'] < self.topK]
            #train_log_topK = mg
            train_log_topK = mg[mg['use']==1]
            select_tag = train_log_topK['rankPosition'].apply(lambda x: 1 if x < self.topK else 0)
            train_log_topK['isSelect'] = select_tag
            #train_log_out_topK = train_log[train_log['rankPosition'] >= self.topK]

            if self.sparse_tag:
                input_train = Wrap_Dataset_clickLog2(doc_tensor=get_sparse_feature(train_log_topK['feature'].to_list()), 
                                                click_tensor = torch.Tensor(train_log_topK['isClick'].to_list()),
                                                select_tensor =  torch.Tensor(train_log_topK['isSelect'].to_list()),
                                                ips_tensor = torch.Tensor(train_log_topK['ips_weight_for_train'].to_list()))
            else:
                input_train = Wrap_Dataset_clickLog2(doc_tensor=torch.Tensor(train_log_topK['feature'].to_list()), 
                                                click_tensor = torch.Tensor(train_log_topK['isClick'].to_list()), 
                                                select_tensor =  torch.Tensor(train_log_topK['isSelect'].to_list()),
                                                ips_tensor = torch.Tensor(train_log_topK['ips_weight_for_train'].to_list()))
            ips_input_train_loader = DataLoader(input_train, 
            ...
            ips_model = rank_model2_tobit(self.in_size, self.hidden_size, self.drop_out)
            ...
            for epoch in range(self.epoch):
                ...
                for _id, batch in enumerate(ips_input_train_loader):
                    ...
                    Tobit_lossfunc = Tobit_MLELoss(weight=batch[3],C=self.C,rho=self.rho)
                    var_val, output_3, output_select = ips_model(batch[0])
                    output_3 = output_3.view(batch[0].size(0))
                    output_select = output_select.view(batch[0].size(0))
                    #train_loss = MSE_lossfunc(var_val, output_3, batch[1])
                    train_loss = Tobit_lossfunc(var_val, output_3, output_select, batch[1], batch[2])
                    train_loss.backward()
                    ips_optim.step()
                    loss_log.append(train_loss.item())
            ...
            if self.use_cuda:
                # train_predict = ips_model(torch.Tensor(train_log['feature']).cuda())
                # vali_predict = ips_model(torch.Tensor(vali_log['feature']).cuda())
                train_var, train_predict, _ = ips_model(torch.Tensor(train_log['feature']).cuda())
                vali_var, vali_predict, _ = ips_model(torch.Tensor(vali_log['feature']).cuda())
            else:
                train_var, train_predict, _ = ips_model(torch.Tensor(train_log['feature']).cpu())
                vali_var, vali_predict, _ = ips_model(torch.Tensor(vali_log['feature']).cpu())               
            train_predict = train_predict.cpu()
            train_predict = train_predict.view(-1).tolist()
            ...
            #train_log['estimate_mix'] = train_log.apply(lambda row: row['estimate_score'] - np.sqrt(row['estimate_var']))
            vali_log['estimate_score'] = vali_predict
            vali_log['estimate_var'] = vali_var
            #vali_log['estimate_mix'] = vali_log.apply(lambda row: row['estimate_score'] - np.sqrt(row['estimate_var']))
            dr_score_train = []
            lamda_train = []
            label_train = []
            for _, row in train_log.iterrows(): 
    ...
    def _check(row):
        if row['click_sum']==0:
            return 1
        elif row['click_sum']!=0 and row['isClick']==1:
            return 1
        else:
            return 0
