...
class Heckman(Learning_Alg):
    def __init__(self, args):
        super(Heckman, self).__init__(args)
    
    def train(self):
        print('Load data...')
        self.test_tool, self.in_size = super(Heckman, self)._load_test_and_get_tool()
        self.vali_tool = super(Heckman, self)._load_vali_and_get_tool()
        input_train_loader = self._load_and_wrap_train()
        print('Train Heckman model...')
    ...
    def _probit(self, Y, X):   
        ...
        clf = LogisticRegression(solver='lbfgs', max_iter=self.epoch, verbose=1, random_state=self.randseed).fit(X, Y)
